...
class Data:
    ...
    def __init__(self, data_dir, device, reverse=False):
    ...
    #         sparse_matrix= self.normalization(sparse_matrix).to(self.device)
    #         adjacencies.append(sparse_matrix)
    #     return adjacencies


    def get_adj_agg_hr(self, alpha):
        """
        :param alpha: alpha
        :return: tensor: Adjacency matrix agg E*R
        """
        dia_columns = [i for i in range(self.relations_num)]
        dia_value = [alpha for i in range(self.relations_num)]
        dense_matrix_sum = torch.zeros(self.entities_num, self.relations_num).to(self.device)
        for i in range(self.entities_num):
            ...
            for h, r, t in self.train_data_id:
                if i == h:
                    ...
                    columns.append(r)
                    values.append(1.0 - alpha)
            dia_rows = [i for j in range(self.relations_num)]
            rows = rows + dia_rows
            columns = columns + dia_columns
            values = values + dia_value
            # 计算度
            column_indices = torch.LongTensor(columns).to(self.device)
            degrees = torch.bincount(column_indices, minlength=self.entities_num)
            # 计算D^-0.5
            degree_rsq = torch.pow(degrees, -0.5).to(self.device)
            # 构造归一化后的稀疏矩阵
            degree_rsq_values = degree_rsq[column_indices]
            normalized_values = degree_rsq_values * torch.FloatTensor(values).to(self.device) * degree_rsq[column_indices]
            sparse_matrix = torch.sparse_coo_tensor(torch.LongTensor([rows, columns]).to(self.device), normalized_values, [self.entities_num, self.relations_num])
            dense_matrix = sparse_matrix.to_dense()
            del sparse_matrix  # 释放显存
            torch.cuda.empty_cache()  # 清除未使用显存
            dense_matrix_sum += dense_matrix
        return dense_matrix_sum
    ...
    def get_adjacencies(self, alpha):
        ...
        for i in range(self.relations_num):
            ...
            values = values + dia_value
            # # 计算度
            row_indices = torch.LongTensor(rows).to(self.device)
            degrees = torch.bincount(row_indices, minlength=self.entities_num)
            # 计算D^-0.5
            degree_rsq = torch.pow(degrees, -0.5).to(self.device)
            # 构造归一化后的稀疏矩阵
            degree_rsq_values = degree_rsq[row_indices]
            normalized_values = degree_rsq_values * torch.FloatTensor(values).to(self.device) * degree_rsq[row_indices]
            sparse_matrix = torch.sparse_coo_tensor(torch.LongTensor([rows, columns]).to(self.device), normalized_values, [self.entities_num, self.entities_num])
            # sparse_matrix= self.normalization(sparse_matrix).to(self.device)
            adjacencies.append(sparse_matrix)
            del sparse_matrix  # 释放显存
            torch.cuda.empty_cache()  # 清除未使用显存
        A = self.get_adj_agg_hr(alpha)
...
# data=Data(data_dir="data/umls/", reverse=False)
# adjacencies = data.get_adjacencies(alpha=0.5)
# test_data, test_data_num, test_hrt = data.load_data("data/umls/", "test")
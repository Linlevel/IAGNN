...
from models.iagnn import *


class RunModel:

    def __init__(self, data: Data, modelname,device, optimizer_method="Adam", learning_rate=0.001, ent_vec_dim=200,
                 rel_vec_dim=200, num_iterations=100, batch_size=128, decay_rate=0., input_dropout=0., drop_rate=0., dropout=0.5, alpha_encode=0.5,
                 hidden_dropout=0., feature_map_dropout=0., in_channels=1, out_channels=32, filt_h=3, filt_w=3,
                 label_smoothing=0., num_to_eval=10, get_best_results=True, get_complex_results=True, alpha=0.5,
        ...
        self.kwargs = {"input_dropout": input_dropout, "hidden_dropout": hidden_dropout, "drop_rate": drop_rate, "dropout": dropout, "alpha": alpha_encode,
        ...
        self.adjacencies, self.A = data.get_adjacencies(alpha)
    ...
    def evaluate(self, model, eval_data, iteration):
        rank_filt = []
        hits_filt = [[] for i in range(10)]

        for batch_data, batch_num in self.data.get_batch_eval_data(self.batch_size, eval_data):
            head_id = batch_data[:, 0].to(self.device)
            rela_id = batch_data[:, 1].to(self.device)
            tail_id = batch_data[:, 2].to(self.device)

            embedded_ent, embedded_rel = model.hgn(self.adjacencies, self.A)
            pred = model.forward(head_id, rela_id, embedded_ent, embedded_rel)
        ...
        if self.get_best_results:
            ...
            logging.info(str(iteration + 1) + '\t' + str('%.2f' % (filt_mr)) + '\t' + str('%.3f' % (filt_mrr)) + '\t' +
                         str('%.3f' % (filt_hits_1)) + '\t' + str('%.3f' % (filt_hits_3)) + '\t' +
                         str('%.3f' % (filt_hits_10)) + '\t' +
                         str('%.2f' % (self.best_mr)) + '\t' + str('%.3f' % (self.best_mrr)) + '\t' +
    ...
    def train_and_eval(self):
        # if self.model_name.lower() == "convtranse":
        #     model = ConvTransE(self.data, self.ent_vec_dim, self.rel_vec_dim, **self.kwargs)
        # elif self.model_name.lower() == "sacn":
        #     model = SACN(self.data, self.ent_vec_dim, self.rel_vec_dim, **self.kwargs)
        if self.model_name.lower() == "iagnn":
            model = IAGNN(self.data, self.ent_vec_dim, self.rel_vec_dim, self.device, **self.kwargs)
        ...
        for iteration in range(self.num_iterations):
            ...
            for batch_data, batch_target in self.data.get_batch_train_data(self.batch_size):
                ...
                optimizer.zero_grad()
                # print('batch_data',batch_data.shape)#torch.Size([128, 2])
                # print('batch_target',batch_target.shape)#torch.Size([128, 104])
                head_id = batch_data[:, 0].to(self.device)
                rela_id = batch_data[:, 1].to(self.device)
                ...
                if self.label_smoothing:
                    batch_target = ((1.0 - self.label_smoothing) * batch_target) + (
                            self.label_smoothing / batch_target.size(1))
            ...
            logging.info('Iteration:' + str(iteration) + '   ' + 'epoch loss: {:.5f}'.format(epoch_loss) +
                         '   ' + 'time cost: {:.3f}'.format(end_time - begin_time))
            ...
            with torch.no_grad():
                if (iteration + 1) % self.num_to_eval == 0:
                    if self.get_complex_results:
                        ...
                        # self.evaluate(model, self.data.valid_data_id)
    ...
    def get_complex_result(self, model, hr_t_data, tr_h_data):
        hr_t_hits_10, hr_t_hits_3, hr_t_hits_1, hr_t_mr, hr_t_mrr = self.evaluate(model, hr_t_data)
        tr_h_hits_10, tr_h_hits_3, tr_h_hits_1, tr_h_mr, tr_h_mrr = self.evaluate(model, tr_h_data)
